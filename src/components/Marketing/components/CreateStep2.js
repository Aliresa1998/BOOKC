...
const CreateStep2 = ({ providePatchCampaignData2, ...props }) => {
    ...
    const handleOk = async (values) => {
        ...
        // Oral Health
        if (oralHealthType && oralHealthRanges) {
            criteria_item.push({
                criteria_type: "oral_health",
                criteria_value: oralHealthType,
                int_range_start: oralHealthRanges[0],
                int_range_end: oralHealthRanges[1]
            });
        }
    ...
    const handleOralHealthRangesChange = (checkedValues) => {
        const parsedRanges = checkedValues.map((value) => {
            const [start, end] = value.split('-').map(Number);
            return { start, end };
        });
        setOralHealthRanges(parsedRanges);
    };
