...
const get_payment_requests = async (page_size, page, search_term, office) => {
  const myHeaders = Object.assign(
    { "Content-Type": "application/json" },
    authHeader()
  );
  ...
  const req = new Request(
    config.apiGateway.URL +
    `/billpay/paymentrequests/?page=${page}&page_size=${page_size}&search=${search_term}&office_id=${office}`,
    {
      method: "GET",
      headers: myHeaders,
    }
...
const createGuarantor = async (
  ...
  const req = new Request(
    config.apiGateway.URL +
    "/clinics/guarantor-lc/?office_id=" +
    localStorage.getItem("selectedOffice"),
    {
      body: raw,
      method: "POST",
...
const getProcedureList = async (page) => {
  ...
  const req = new Request(
    config.apiGateway.URL + `/clinics/list-procedure/?page=` + page + "&office_id=" + localStorage.getItem("selectedOffice"),
    {
      method: "GET",
      headers: myHeaders,
    }
